...
import { noCoverImage, noProfileImageUser } from "app/images";
...
export default function ProfileHeaderUser() {
    ...
    useEffect(() => {
        async function loadUser() {
            ...
            setProfileData({
                ...
                createdAt: user?.createdAt ? new Date(user.createdAt) : new Date(), // Ajuste para usar a data do usuário se existir
        ...
        loadUser();
    }, [user]); // Dependência em 'user' para recarregar quando o contexto do usuário mudar
    ...
    const handleSaveNewPassword = async(newPassword: string) => {
        try {
            ...
            if (!email) {
                alert("Email do usuário não disponível para alteração de senha.");
                return;
            }
    ...
    const profileImageUrl = profileData.profileImage || noProfileImageUser;
    const coverImageUrl = profileData.coverImage || noCoverImage;

    if (isLoading || !user) { // Verifica se 'user' do contexto está carregando ou é nulo
        return (
            <div className="bg-white rounded-lg shadow p-4 text-center">
                <h3 className="text-base font-medium">Carregando perfil do usuário...</h3>
            </div>
        );
    }
            ...
            {selectedImage && (
                <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
                    <div ref={modalRef} className="relative bg-white rounded-lg shadow-lg max-w-full max-h-full">
                        ...
                        <a
                            href={selectedImage}
                            download={getFileNameFromUrl(selectedImage, profileData.name || "Perfil_Usuario")}
                        ...
                        >
                            <img
                                ref={imageRef}
                                src={selectedImage}
                                alt={`Imagem de perfil ou capa de ${profileData.name}`}
