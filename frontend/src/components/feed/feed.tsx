...
export default function Feed({ reloadTrigger }: { reloadTrigger: number }) {
  ...
  async function handleLike(postId: number) {
    // --- 1. Guard Clauses (Validações iniciais) ---
    if (!loggedInEntity?.id) {
      console.warn("Nenhuma entidade logada para realizar a ação.");
      return;
    }
    ...
    const getLikerId = (like: any): number | undefined =>
      like.user?.id ?? like.ong?.id;

    // --- 2. Preparação da Ação ---
    const isUnlikeAction = post.userLiked;
    let method: string;
    let apiUrl: string;

    if (isUnlikeAction) {
      const likeToRemove = post.likes.find(
        (like) => getLikerId(like) === loggedInEntity.id
      );
      if (!likeToRemove) {
        console.warn("Like a ser removido não encontrado no estado local.");
        return; // Sai se não encontrar o like para remover
      }
      method = "DELETE";
      apiUrl = `/api/posts/${postId}/likes/${likeToRemove.id}`;
    } else {
      method = "POST";
      apiUrl = `/api/posts/${postId}/likes`;
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          errorData.message ||
            `Falha na operação de ${isUnlikeAction ? "descurtir" : "curtir"}`
        );
      }
      ...
      if (isUnlikeAction) {
        ...
        // Adiciona o novo like retornado pela API
      ...
      updatedPosts[postIndex] = {
        ...post,
        likes: updatedLikes,
        userLiked: !isUnlikeAction, // Inverte o estado anterior
      };
      setPosts(updatedPosts);
    } catch (err) {
      console.error(
        `Erro ao ${isUnlikeAction ? "descurtir" : "curtir"} (frontend catch):`,
        err
      );
    }
  }
  ...
  async function handleComment(postId: number, event?: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (
          currentVisible < updatedCommentsLength &&
          currentVisible === comments[postId]?.length
        ) {
          setVisibleComments((prev) => ({
            ...prev,
  ...
  async function handleEditComment(postId: number, commentId: number) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const updateCommentsInPost = (post: Post): Post => {
          // Se não for o post que queremos alterar, retorne ele sem modificações.
          if (post.id !== postId) {
            return post;
          }
          // Se for o post correto, mapeie seus comentários e atualize o que mudou.
          return {
            ...post,
        ...
        setComments((prev) => {
          const updatedCommentsForPost = prev[postId].map((comment) =>
            comment.id === commentId ? updatedComment : comment
          );
          return { ...prev, [postId]: updatedCommentsForPost };
        });

        setPosts((prev) => prev.map(updateCommentsInPost));
  ...
  async function handleDeleteComment(postId: number, commentId: number) {
    ...
    try {
      ...
      if (res.ok) {
        const removeCommentFromPost = (post: Post): Post => {
          // Se não for o post que queremos alterar, retorne ele sem modificações.
          if (post.id !== postId) {
            return post;
          }
          // Se for o post correto, retorne uma nova versão com o comentário filtrado.
          return {
            ...post,
        ...
        setPosts((prev) => prev.map(removeCommentFromPost));
  ...
  const processFetchedPosts = (posts: any[], loggedInEntity: any): Post[] => {
    if (!posts || !Array.isArray(posts)) return [];

    // Helper interno para verificar se um post foi curtido pela entidade
    const isLikedByEntity = (like: any): boolean => {
      const likerId = like.user?.id ?? like.ong?.id;
      const likerRole = like.user?.role ?? like.ong?.role;
      return (
        likerId === loggedInEntity?.id && likerRole === loggedInEntity?.role
      );
    };
    ...
    return sortedPosts.map((post) => ({
      ...post,
      userLiked: post.likes.some(isLikedByEntity),
    }));
  };

  const initializeAssociatedStates = (posts: Post[]) => {
    const commentDrafts: Record<number, string> = {};
    const expandedComments: Record<number, boolean> = {};
    const showCommentInput: Record<number, boolean> = {};
    const visibleComments: Record<number, number> = {};
    const hasMoreComments: Record<number, boolean> = {};
    const comments: Record<number, Comment[]> = {};

    posts.forEach((post) => {
      commentDrafts[post.id] = "";
      expandedComments[post.id] = false;
      showCommentInput[post.id] = false;
      visibleComments[post.id] = 2; // Número padrão de comentários visíveis
      hasMoreComments[post.id] = (post.comments?.length || 0) > 2;
  ...
  useEffect(() => {
    async function loadPosts() {
      ...
      try {
        ...
        const processedPosts = processFetchedPosts(
          fetchedPosts,
          loggedInEntity
        );
        const initialStates = initializeAssociatedStates(processedPosts);

        setPosts(processedPosts);
        setCommentDrafts(initialStates.commentDrafts);
        setExpandedComments(initialStates.expandedComments);
        setShowCommentInput(initialStates.showCommentInput);
        setVisibleComments(initialStates.visibleComments);
        setHasMoreComments(initialStates.hasMoreComments);
    ...
    <div className="space-y-4">
      {posts.map((post) => {
        ...
        if (!postAuthor) {
          ...
          console.warn(
            `Post ${post.id} não possui um autor (user ou ong) válido.`
          );
          return (
            <div
              key={post.id}
        ...
        const authorProfileImage =
          ...
          (post.user ? noProfileImageUser : noProfileImageONG);
        ...
        return (
          <div key={post.id} className="bg-white rounded-lg shadow">
              ...
              <div className="flex justify-between">
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    {postAuthor.city && (
                      <p className="text-sm text-gray-500">{postAuthor.city}</p>
                    )}
            ...
            <div className="flex w-full border-b">
              <button
                className={`flex-1 flex items-center justify-center py-3 hover:bg-gray-100 transition duration-200 ${
                  post.userLiked
              ...
              >
                <svg
                  ...
                  className={`h-5 w-5 mr-2 transition-transform duration-200 ${
                    post.userLiked ? "scale-110 text-blue-600" : ""
                  }`}
                >
                  <path d="M7 10v12" />
            ...
            <div className="px-4 py-2">
                ...
                >
                  <div className="h-8 w-8 rounded-full overflow-hidden flex-shrink-0">
                    {loggedInEntity?.profileImage ? (
                      ...
                      <div className="h-full w-full bg-gray-300 flex items-center justify-center">
                        <span className="text-gray-600 font-semibold">
                          {loggedInEntity?.name?.charAt(0) ||
                            loggedInEntity?.nameONG?.charAt(0)}
                        </span>
                      </div>
                    )}
                  </div>
              ...
              {expandedComments[post.id] && comments[post.id]?.length > 0 && (
                <div className="space-y-3 mt-2">
                  {comments[post.id]
                    .slice(0, visibleComments[post.id] || 2)
                    .map((comment) => {
                      ...
                      if (!commentAuthor) {
                        console.warn(
                          `Comentário ${comment.id} não possui um autor (user ou ong) válido.`
                        );
                        return null;
                      }
                      ...
                      const authorProfileImage =
                        ...
                        (comment.user ? noProfileImageUser : noProfileImageONG);
