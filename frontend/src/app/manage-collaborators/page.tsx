...
export default function ManageCollaboratorsPage() {
  ...
  const [usersNotAssociated, setUsersNotAssociated] = useState<UserCollaborator[]>([]);
  const [pendingInvites, setPendingInvites] = useState<UserInvite[]>([]);
  const [acceptedCollaborators, setAcceptedCollaborators] = useState<UserInvite[]>([]);
  ...
  const fetchAllCollaboratorLists = async () => {
    ...
    try {
      ...
      if (!allOngRelationsRes.ok) throw new Error(`Falha ao carregar relações da ONG: ${(await allOngRelationsRes.json()).error}`);
      const allOngRelations: UserInvite[] = await allOngRelationsRes.json();

      // IDs dos usuários que já têm ALGUMA relação com esta ONG
      const usersWithAnyRelationIds = new Set(allOngRelations.map(relation => relation.userId));

      // 2. Fetch de Usuários Não Associados (na visão global do sistema, ongId é null)
      const nonAssociatedUsersGlobalRes = await fetch(`/api/users/not-associated-with-ongs`);
      if (!nonAssociatedUsersGlobalRes.ok) throw new Error(`Falha ao carregar não associados globalmente: ${(await nonAssociatedUsersGlobalRes.json()).error}`);
      const nonAssociatedUsersGlobal: UserCollaborator[] = await nonAssociatedUsersGlobalRes.json();

      // 3. Filtrar usersNotAssociated:
      // Apenas aqueles que não têm NENHUMA relação com esta ONG.
      const trulyNotAssociated = nonAssociatedUsersGlobal.filter(userItem =>
        !usersWithAnyRelationIds.has(userItem.id) // Remove usuários que já têm alguma relação (pendente, aceita, rejeitada)
      );
      setUsersNotAssociated(trulyNotAssociated);


      // 4. Separar e preencher as outras listas a partir de 'allOngRelations'
      setPendingInvites(allOngRelations.filter(rel => rel.status === 'INVITE_PENDING_ONG_TO_USER'));
      setAcceptedCollaborators(allOngRelations.filter(rel => rel.status === 'ACCEPTED'));
      setRejectedInvites(allOngRelations.filter(rel => rel.status === 'REJECTED_BY_USER'));

    } catch (err: any) {
      console.error("Erro ao carregar listas de colaboradores:", err);
      setError(err.message || "Não foi possível carregar as listas de colaboradores.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSendInvite = async (userToInvite: UserCollaborator) => {
    if (!ong || !ong.id || !ong.nameONG || !ong.emailONG) {
      alert("Erro: Dados da ONG logada incompletos. Não é possível enviar convite.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(JSON.stringify(errorData.error.message) || `Falha ao registrar convite na API (Status: ${response.status})`);
      }
  ...
  if (isLoading) {
    return <div className="text-center p-4">Carregando listas de colaboradores...</div>;
  }
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <main>
        <h1 className="text-2xl font-bold mb-6">Gerenciar Colaboradores para {ong?.nameONG}</h1>

        {error && <div className="mb-4 p-3 bg-red-100 text-red-700 rounded-md">{error}</div>}

        {/* 1. Colaboradores Não Associados */}
        <div className="bg-white shadow rounded-lg p-4 mb-6">
          <h2 className="text-xl font-semibold mb-4">1. Colaboradores Não Associados</h2>
          {usersNotAssociated.length === 0 ? (
            <p className="text-gray-600">Nenhum usuário disponível para convidar.</p>
          ) : (
            <ul className="divide-y divide-gray-200">
              {usersNotAssociated.map((userItem) => (
                <li key={userItem.id} className="py-4 flex justify-between items-center">
                  <div>
                    <p className="text-lg font-medium text-gray-900">{userItem.name}</p>
                  ...
                  >
                    {invitingUserId === userItem.id ? 'Enviando...' : 'Enviar Convite'}
                  </button>
                </li>
              ))}
            </ul>
        ...
        <div className="bg-white shadow rounded-lg p-4 mb-6">
          <h2 className="text-xl font-semibold mb-4">2. Convites Enviados (Pendentes)</h2>
            ...
            <ul className="divide-y divide-gray-200">
              {pendingInvites.map((invite) => (
                <li key={invite.id} className="py-4 flex justify-between items-center">
                  <div>
                    <p className="text-lg font-medium text-gray-900">{invite.user.name}</p>
        ...
        <div className="bg-white shadow rounded-lg p-4 mb-6">
          <h2 className="text-xl font-semibold mb-4">3. Colaboradores Atuais</h2>
          {acceptedCollaborators.length === 0 ? (
            <p className="text-gray-600">Nenhum colaborador aceito nesta ONG.</p>
          ) : (
            <ul className="divide-y divide-gray-200">
        ...
        <div className="bg-white shadow rounded-lg p-4 mb-6">
          ...
          {rejectedInvites.length === 0 ? (
            <p className="text-gray-600">Nenhum convite rejeitado no momento.</p>
          ) : (
            <ul className="divide-y divide-gray-200">
